...
__global__ void preprocessCUDA(
	...
	glm::vec4* dL_drot,
	float *dL_dtau,
	float3 *dL_dgeom_points,
	float3 *dL_dgeom_motions,
	glm::vec3* dL_dmotion
	)
{
	auto idx = cg::this_grid().thread_rank();
	...
	// TODO: update dL_dtau from points
	// for (int i = 0; i < 3; i++) {
	...
	float3 dL_dp = dL_dgeom_points[idx];
	dL_dmeans[idx].x += dL_dp.x * viewmatrix[0] + dL_dp.y * viewmatrix[1] + dL_dp.z * viewmatrix[2];
    dL_dmeans[idx].y += dL_dp.x * viewmatrix[4] + dL_dp.y * viewmatrix[5] + dL_dp.z * viewmatrix[6];
    dL_dmeans[idx].z += dL_dp.x * viewmatrix[8] + dL_dp.y * viewmatrix[9] + dL_dp.z * viewmatrix[10];
	// TODO: update dL_dtau from points

	// Gradient from points
	float3 dL_dm = dL_dgeom_motions[idx];
	dL_dmotion[idx].x += dL_dm.x * viewmatrix[0] + dL_dm.y * viewmatrix[1] + dL_dm.z * viewmatrix[2];
	dL_dmotion[idx].y += dL_dm.x * viewmatrix[4] + dL_dm.y * viewmatrix[5] + dL_dm.z * viewmatrix[6];
	dL_dmotion[idx].z += dL_dm.x * viewmatrix[8] + dL_dm.y * viewmatrix[9] + dL_dm.z * viewmatrix[10];
...
renderCUDA(
	...
	float3* __restrict__ dL_dmean2D,  // why float3? not float2?
	float4* __restrict__ dL_dconic2D,
	float* __restrict__ dL_dopacity,
	float* __restrict__ dL_dcolors,
	const float3* __restrict__ points,  // geomState points
	const float* __restrict__ dL_dpixels_point,  // Backproped gradient from the loss
	float3* __restrict__ dL_dgeom_point,  // Gradient to be calculated for geomState points
	const float3* __restrict__ motions,  // geomState motions
	const float* __restrict__ dL_dpixels_motion,  // Backproped gradient from the loss
	float3* __restrict__ dL_dgeom_motion  // Gradient to be calculated for geomState motions
)
{
	...
	__shared__ float3 collected_points[BLOCK_SIZE];
	__shared__ float3 collected_motions[BLOCK_SIZE];
	...
	__shared__ float3 dL_dpoints_shared[BLOCK_SIZE];
	__shared__ float3 dL_dmotions_shared[BLOCK_SIZE];
	...
	float accum_rec[C] = { 0 };
	float dL_dpixel[C] = { 0 };
	float3 accum_rec_point = {0.0f, 0.0f, 0.0f};
	float3 dL_dpixel_point = {0.0f, 0.0f, 0.0f};  // defined array instead of float3, for the for loops below.
	float3 accum_rec_motion = {0.0f, 0.0f, 0.0f};
	float3 dL_dpixel_motion = {0.0f, 0.0f, 0.0f};

	// Save gradients for inside pixels.
	if (inside) {
		#pragma unroll
		for (int i = 0; i < C; i++) {
		...
		dL_dpixel_point = {
			dL_dpixels_point[0 * H * W + pix_id],
			dL_dpixels_point[1 * H * W + pix_id],
			dL_dpixels_point[2 * H * W + pix_id]
		};
		dL_dpixel_motion = {
			dL_dpixels_motion[0 * H * W + pix_id],
			dL_dpixels_motion[1 * H * W + pix_id],
			dL_dpixels_motion[2 * H * W + pix_id]
		};
	}

	float last_alpha = 0.f;
	float last_color[C] = { 0.f };
	float3 last_point = {0.0f, 0.0f, 0.0f};
	float3 last_motion = {0.0f, 0.0f, 0.0f};
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		if (range.x + progress < range.y)
		{
			...
			collected_points[tid] = points[coll_id];
			collected_motions[tid] = motions[coll_id];
		...
		for (int j = 0; j < min(BLOCK_SIZE, toDo); j++) {
			...
			// Point gradients
			const float3 point_j = collected_points[j];
			accum_rec_point = skip ? accum_rec_point : last_alpha * last_point + (1.f - last_alpha) * accum_rec_point;
			last_point = skip ? last_point : point_j;
			dL_dalpha += (point_j.x - accum_rec_point.x) * (dL_dpixel_point.x);
			dL_dalpha += (point_j.y - accum_rec_point.y) * (dL_dpixel_point.y);
			dL_dalpha += (point_j.z - accum_rec_point.z) * (dL_dpixel_point.z);
			dL_dpoints_shared[tid].x = skip ? 0.0f : dchannel_dcolor * dL_dpixel_point.x;
			dL_dpoints_shared[tid].y = skip ? 0.0f : dchannel_dcolor * dL_dpixel_point.y;
			dL_dpoints_shared[tid].z = skip ? 0.0f : dchannel_dcolor * dL_dpixel_point.z;

			// Motion gradients
			const float3 motion_j = collected_motions[j];
			accum_rec_motion = skip ? accum_rec_motion : last_alpha * last_motion + (1.f - last_alpha) * accum_rec_motion;
			last_motion = skip ? last_motion : motion_j;
			dL_dalpha += (motion_j.x - accum_rec_motion.x) * (dL_dpixel_motion.x);
			dL_dalpha += (motion_j.y - accum_rec_motion.y) * (dL_dpixel_motion.y);
			dL_dalpha += (motion_j.z - accum_rec_motion.z) * (dL_dpixel_motion.z);
			dL_dmotions_shared[tid].x = skip ? 0.0f : dchannel_dcolor * dL_dpixel_motion.x;
			dL_dmotions_shared[tid].y = skip ? 0.0f : dchannel_dcolor * dL_dpixel_motion.y;
			dL_dmotions_shared[tid].z = skip ? 0.0f : dchannel_dcolor * dL_dpixel_motion.z;
			...
			if (tid == 0) {
				...
				float3 dL_dpoints_acc = dL_dpoints_shared[0];
				float3 dL_dmotions_acc = dL_dmotions_shared[0];
				...
				atomicAdd(&dL_dgeom_point[global_id].x, dL_dpoints_acc.x);
				atomicAdd(&dL_dgeom_point[global_id].y, dL_dpoints_acc.y);
				atomicAdd(&dL_dgeom_point[global_id].z, dL_dpoints_acc.z);
				atomicAdd(&dL_dgeom_motion[global_id].x, dL_dmotions_acc.x);
				atomicAdd(&dL_dgeom_motion[global_id].y, dL_dmotions_acc.y);
				atomicAdd(&dL_dgeom_motion[global_id].z, dL_dmotions_acc.z);
			}
		}
	}
}
...
void BACKWARD::render(
	...
	const float* dL_dpixels,  // dL_dcolor
	float3* dL_dmean2D,
	float4* dL_dconic2D,
	float* dL_dopacity,
	float* dL_dcolors,
...
)
{
	renderCUDA<NUM_CHANNELS> << <grid, block >> >(
		...
		dL_dopacity,
		dL_dcolors,
		points,  // geomState points
		dL_dpixels_point,  // Backproped gradient from the loss
		dL_dgeom_point,  // Gradient to be calculated for geomState points
		motions,  // geomState motions
		dL_dpixels_motion,  // Backproped gradient from the loss
		dL_dgeom_motion  // Gradient to be calculated for geomState motions
	);
}