...
@dataclass
class LossPointmapRenderedCfgWrapper:
    pointmap_rendered: LossPointmapRenderedCfg


class LossPointmapRendered(Loss[LossPointmapRenderedCfg, LossPointmapRenderedCfgWrapper]):
    def forward(
        self,
        ...
        # Rendered point loss on all rendered views.
        assert pred_points.size() == target.size()  # b v c h w

        epe = torch.sqrt(((pred_points - target) ** 2).sum(2, keepdim=True))
