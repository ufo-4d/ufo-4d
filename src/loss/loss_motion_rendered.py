...
@dataclass
class LossMotionRenderedCfgWrapper:
    motion_rendered: LossMotionRenderedCfg


class LossMotionRendered(Loss[LossMotionRenderedCfg, LossMotionRenderedCfgWrapper]):
    def forward(
        self,
        ...
        b, v_render, _, _, _ = target.size()
        v_model = 1 if self.cfg.view_mode == 'single_view' else v_render

        assert prediction.motion_rendered is not None
        motion = prediction.motion_rendered
        ...
        if self.cfg.scale_invariant and self.cfg.scale_invariant_apply_after < global_step:
            ...
            target = target / (batch["target"]["pointmap_scale"].reshape(b, 1, 1, 1, 1).detach())
            motion = motion / (batch["pred"]["pointmap_scale"].reshape(b, 1, 1, 1, 1).detach())

        target = target[:, :v_model, ...]
        motion = motion[:, :v_model, ...]
        mask = mask[:, :v_model, ...]
        # Rendered motion loss on all rendered views.
