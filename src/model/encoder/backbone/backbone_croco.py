...
class AsymmetricCroCo(CroCoNet):
    ...
    def __init__(self, cfg: BackboneCrocoCfg, d_in: int) -> None:
        ...
        if self.pose_embed_loc == 'encoder':
            self.pose_token = nn.Parameter(
                torch.randn(1, 1, 1024) * 0.02, requires_grad=True
            ) # b, v, dim
        elif self.pose_embed_loc == 'decoder':
            raise ValueError('Pose emb decoder is not implemented.')
    ...
    def _encode_image(self, image, true_shape, intrinsics_embed=None, pose_embed=None):
        ...
        if pose_embed is not None:
            x = torch.cat((x, pose_embed), dim=1)
            add_pose = pos[:, 0:1, :].clone()
            add_pose[:, :, 0] += (pos[:, -1, 0].unsqueeze(-1) + 1)
            pos = torch.cat((pos, add_pose), dim=1)
    ...
    def _encode_image_pairs(
        ...
        if img1.shape[-2:] == img2.shape[-2:]:
            out, pos, _ = self._encode_image(torch.cat((img1, img2), dim=0),
                                             ...
                                             torch.cat((pose_embed1, pose_embed2), dim=0) if pose_embed1 is not None else None,)
    ...
    def _encode_symmetrized(self, view1, view2, force_asym=False):
        ...
        pose_embed1 = view1.get('pose_embed', None)
        pose_embed2 = view2.get('pose_embed', None)

        if force_asym or not is_symmetrized(view1, view2):
            feat1, feat2, pos1, pos2 = self._encode_image_pairs(
                img1, img2, shape1, shape2, intrinsics_embed1, intrinsics_embed2, pose_embed1, pose_embed2)
        else:
            raise ValueError('intrinsic embed and pose embed is not done.')
    ...
    def forward(self,
        ...
        if self.pose_embed_loc == 'encoder':
            view1['pose_embed'] = self.pose_token.expand(b, -1, -1)
            view2['pose_embed'] = self.pose_token.expand(b, -1, -1)

        if symmetrize_batch:
            raise ValueError("Not supported for intrinsic and pose embed.")
        ...
        # Discard intrinsic token from the token lists.
        if self.intrinsics_embed_loc == 'encoder' and self.intrinsics_embed_type == 'token':
            
            dec1, dec2 = list(dec1), list(dec2)
            
            if self.pose_embed_loc == 'encoder':
                # [feat..., intrinsics, pose], keep the pose token
                for i in range(len(dec1)):
                    dec1[i] = torch.cat([dec1[i][:, :-2], dec1[i][:, -1:]], dim=1)
                    dec2[i] = torch.cat([dec2[i][:, :-2], dec2[i][:, -1:]], dim=1)
            else:
                # [feat..., intrinsics]
                for i in range(len(dec1)):
                    dec1[i] = dec1[i][:, :-1]
